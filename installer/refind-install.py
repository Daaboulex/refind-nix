...
def config(*path: str) -> Any | None:
    result = install_config
    try:
        for component in path:
            result = result[component]
    except KeyError:
        raise KeyError(f"refind config missing key: {'.'.join(path)}") from None
    return result
...
def get_system_path(
    profile: str = "system", gen: int | None = None, spec: str | None = None
) -> str:
...
def check_signing_prerequisites() -> None:
    ...
    if not os.access(sbctl, os.X_OK):
        raise RuntimeError(
            f"refind-install: signing enabled but sbctl is not executable: {sbctl}"
        )
...
def find_partuuid(part_device: str) -> str | None:
...
def validate_path_within(dest: str, base: str) -> None:
    ...
    if not (dest.startswith(base + os.sep) or dest == base):
        raise RuntimeError(f"path escapes target directory: {dest} is outside {base}")
...
def install_bootloader() -> None:
    ...
    if not os.path.ismount(efi_mount):
        if config("graceful"):
            print(
                f"warning: ESP not mounted at {efi_mount}, skipping installation",
                file=sys.stderr,
            )
            return
    ...
    if os.path.exists(kernels_dir):
        for dir, _dirs, files in os.walk(kernels_dir, topdown=True):
            for file in files:
                paths[os.path.join(dir, file)] = False
    ...
    if cpu_family == "x86":
        if config("hostArchitecture", "bits") == 32:
            ...
            raise Exception(
                f"Unsupported x86 variant: {config('hostArchitecture', 'bits')} bits"
            )
    elif cpu_family == "arm":
        if (
    ...
    dest_path = os.path.join(
        refind_dir, boot_file if config("efiRemovable") else efi_file
    )
    ...
    if manage_nvram and config("canTouchEfiVariables"):
        if config("efiRemovable"):
            print(
                "note: efiInstallAsRemovable is true, no need to add EFI NVRAM entry."
            )
        else:
            ...
            if refind_boot_entry:
                ...
                new_matches = re.findall(
                    r"Boot([0-9a-fA-F]{4})\*? rEFInd", create_output
                )
                ...
                try:
                    ...
                    print(
                        f"warning: failed to remove old boot entry {refind_boot_entry}: {e.output}",
                        file=sys.stderr,
                    )
                ...
                if boot_order and new_boot_num:
                    ...
                    try:
                        ...
                        print(
                            f"warning: failed to restore boot order: {e.output}",
                            file=sys.stderr,
                        )
            else:
                subprocess.check_output(
        ...
        try:
            ...
            try:
                ...
                if rc != 0:
                    ...
                    print(
                        f"could not sync {config('efiMountPoint')}: {os.strerror(errno)}",
                        file=sys.stderr,
                    )
            finally:
                os.close(fd)
